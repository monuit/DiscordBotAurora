...
const { ApplicationCommandOptionType, ApplicationCommandType, REST, Routes, ApplicationCommandManager } = require('discord.js');

// Helper function to convert type string to number
function getCommandType(typeString) {
  switch (typeString) {
    case 'CHAT_INPUT':
    ...
    default:
      return ApplicationCommandType.ChatInput; // Default to ChatInput if not specified
  }
}
  ...
  if (!cleared) {
    ...
    command = store.reduce((all, current) => {
      switch (current.name.length) {
        case 1: {
          all.push({
            type: getCommandType(current.type),
            name: current.name[0],
        ...
        case 2: {
          let baseItem = all.find((i) => {
            return i.name == current.name[0] && i.type == getCommandType(current.type)
          });
          if (!baseItem) {
            all.push({
              type: getCommandType(current.type),
